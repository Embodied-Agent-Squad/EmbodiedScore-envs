"""The Isaac Sim 5.1 render worker.

Lineage: the zero-shot line's ``envs/sim_backends/sim_workers/isaac_worker.py``
(billzhao1030/vlnverse_emr_zero_shot), carried through the AgentCanvas
``env_vlnverse`` nodeset.

Runs inside Isaac's bundled python (``$EMBODIEDSCORE_ISAAC_PYTHON``, default
``~/isaacsim5.1/python.sh``), reads length-prefixed msgpack command frames on
stdin, writes response frames on stdout. Human-readable logs go to stderr.

This is the only file in the package that imports ``isaacsim`` / ``omni.*``,
and it is deliberately standalone: it must NOT import anything from the
package (frame IO is duplicated here on purpose), because Isaac's python
does not have the package installed. It needs ``msgpack`` and
``msgpack-numpy`` inside Isaac's python (README "Install").

Deviations from upstream:
  - Added a ``--check`` CLI flag: verifies ``isaacsim`` is importable and
    prints its version, WITHOUT booting SimulationApp — front-loads "is
    Isaac even installed" failures to install time.
  - ``--selftest`` looks scenes up under ``--scene-root`` /
    ``$EMBODIEDSCORE_SCENE_ROOT/vlnverse`` (the package's scene layout).
...
# ---------------------------------------------------------------------------
# Frame I/O (duplicated from backend.py because this script runs in the
# isaac python and cannot import the package).
# ---------------------------------------------------------------------------
...
# claim fd1). Must match backend.STDIO_MAGIC byte-for-byte.
...
def check() -> int:
    ...
    readable error otherwise, so a missing/broken Isaac install fails at
    install time, not first run.
    """
    try:
...
def selftest(scene_id: Optional[str], usd_path: Optional[str], scene_root: Optional[str]) -> int:
    if usd_path is None:
        if scene_root is None and os.environ.get('EMBODIEDSCORE_SCENE_ROOT'):
            scene_root = os.path.join(os.environ['EMBODIEDSCORE_SCENE_ROOT'], 'vlnverse')
        if scene_id is None:
            scene_id = 'kujiale_0003'
        if scene_root is None:
            log('selftest needs --scene-root or EMBODIEDSCORE_SCENE_ROOT (or --usd-path)')
            return 2
        candidates = [
            os.path.join(scene_root, scene_id, 'start_result_navigation.usd'),
            os.path.join(scene_root, scene_id, 'start_result_navigation.usda'),
            os.path.join(scene_root, scene_id, f'{scene_id}.usd'),
            os.path.join(scene_root, scene_id, f'{scene_id}.usda'),
        ]
    ...
    try:
        ...
        out_dir = os.path.join(os.getcwd(), 'debug_outputs', 'sim_worker_selftest')
...
def serve_unix_socket(socket_path: str) -> None:
    ...
    The parent process (the environment, possibly under a debugger)
    connects to this socket after we boot Isaac, so it can attach
...
def main() -> int:
    ...
    if args.selftest:
        return selftest(args.scene_id, args.usd_path, args.scene_root)
