"""The Isaac Sim 5.1 boundary: clients for the out-of-process render worker.

Lineage: the zero-shot line's ``envs/sim_backends/{sim_backend,subprocess_backend}.py``
(billzhao1030/vlnverse_emr_zero_shot), carried through the AgentCanvas
``env_vlnverse`` nodeset.

This process talks to the simulator only through the :class:`SimBackend`
protocol below; all method arguments and return values are numpy arrays or
plain Python types, never Isaac objects. The actual Isaac 5.1 runtime lives
in ``worker.py``, spawned under Isaac's bundled python
(``$EMBODIEDSCORE_ISAAC_PYTHON``, default ``~/isaacsim5.1/python.sh``) and driven
over length-prefixed msgpack frames. This process must NEVER import
``omni.*`` / ``isaacsim``.

Deviations from upstream:
  - Dropped ``LocalIsaacSimBackend`` — an in-process Isaac instance is
    forbidden; only the subprocess / socket clients remain.
  - Dropped ``IsaacPanoramicRenderer`` — legacy shim over a pre-built
    World/Camera pair; nothing here constructs those.
  - Dropped ``_coerce_rgb`` / ``_coerce_depth`` — they were only consumed
    by the two dropped classes; the worker keeps its own duplicates.
  - ``SocketIsaacSimBackend``'s "start the worker first" hint names this
    package's ``worker.py``.
...
class SimBackend(Protocol):
    """Minimal simulator surface used by :class:`IsaacWorld`."""
...
# worker.STDIO_MAGIC byte-for-byte. Socket mode has no shared-fd
...
class SubprocessIsaacSimBackend(_FramedRPCMixin):
    ...
    def __init__(
        ...
        env = os.environ.copy()
        # Strip PYTHONPATH leaks from the parent env so the Isaac 5.1
        # launcher sets up its own paths cleanly.
...
class SocketIsaacSimBackend(_FramedRPCMixin):
    ...
    one warmed-up Isaac worker (slow boot done once) and run the environment
    ...
    def __init__(self, socket_path: str, connect_timeout: float = 30.0) -> None:
        ...
        if not os.path.exists(socket_path):
            raise FileNotFoundError(
                ...
                f'vlnverse/benchmarks/env/sim/worker.py --listen {socket_path}'
            )
