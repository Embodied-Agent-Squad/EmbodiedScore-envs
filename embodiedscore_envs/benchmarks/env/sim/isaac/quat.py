"""Quaternion helpers (w, x, y, z) — numpy only.

Lineage: the zero-shot line's ``envs/_quat.py`` (billzhao1030/vlnverse_emr_zero_shot,
``vlnverse/projects/internutopia_vln_extension/envs/_quat.py``), carried through the
AgentCanvas ``env_vlnverse`` nodeset.
...
- scipy dependency dropped (numpy + stdlib only).
  ...
  **extrinsic** x→y→z rotations, i.e. R = Rz(yaw) @ Ry(pitch) @ Rx(roll).
  ...
  comment. For the yaw-only rotations this package performs the two
  conventions coincide.)
- Same signatures and (w, x, y, z) ordering as upstream.

Parity with scipy is pinned by ``tests/test_freemap.py`` (skipped when scipy
is absent).
"""
