...
from .sim import (BehaviorBody, BehaviorSceneRef, Body, IsaacBody, IsaacSceneRef, LiberoBody, LiberoSceneRef,
...
ENGINES = ("habitat", "isaac", "libero", "robotwin", "robocasa", "behavior")
...
class Episode:
    ...
    episode_id: str
    scene: SceneRef | IsaacSceneRef | LiberoSceneRef | RobotwinSceneRef | RobocasaSceneRef | BehaviorSceneRef
...
class Benchmark:
    ...
    body: Body | IsaacBody | LiberoBody | RobotwinBody | RobocasaBody | BehaviorBody
    actions: tuple[Act, ...]
    splits: tuple[str, ...]
    ...
    def __post_init__(self) -> None:
        ...
        want = {"habitat": Body, "isaac": IsaacBody, "libero": LiberoBody, "robotwin": RobotwinBody,
                "robocasa": RobocasaBody, "behavior": BehaviorBody}[self.engine]
        ...
        if (self.pose and self.engine != "habitat" or self.polar and self.engine != "isaac"
                or self.macro and self.engine not in ("libero", "robotwin", "robocasa", "behavior")):
            raise ValueError(f"{self.name}: pose is a habitat protocol, polar an isaac one, "
                             "macro a manipulation one (libero / robotwin / robocasa / behavior)")
        if not self.line:
