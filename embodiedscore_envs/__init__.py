...
the LIBERO manipulation lines on robosuite 1.4 / MuJoCo in process.
...
The Isaac and LIBERO lines register ``nondeterministic=True``: the episode, the
...
for _b in BENCHMARKS.values():
    register(id=_b.gym_id, entry_point="embodiedscore_envs.benchmarks:build_env",
             max_episode_steps=_b.max_episode_steps, nondeterministic=_b.engine in ("isaac", "libero"),
