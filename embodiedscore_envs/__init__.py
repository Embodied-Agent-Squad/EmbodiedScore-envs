...
RoboCasa / RoboCasa365 kitchen lines on robosuite 1.5 / MuJoCo in process.
...
The Isaac, LIBERO and RoboCasa lines register ``nondeterministic=True``: the episode, the
poses and every fact in ``info`` are reproducible under a seed; the rendered
frames are not bit-identical from one render to the next (Isaac's RTX, and
robosuite's offscreen EGL renderer alike).
"""
...
for _b in BENCHMARKS.values():
    register(id=_b.gym_id, entry_point="embodiedscore_envs.benchmarks:build_env",
             max_episode_steps=_b.max_episode_steps, nondeterministic=_b.engine in ("isaac", "libero", "robocasa"),
