...
RoboTwin bimanual manipulation lines on SAPIEN 3 in process.
...
The Isaac, LIBERO and RoboTwin lines register ``nondeterministic=True``: the
...
for _b in BENCHMARKS.values():
    register(id=_b.gym_id, entry_point="embodiedscore_envs.benchmarks:build_env",
             max_episode_steps=_b.max_episode_steps, nondeterministic=_b.engine in ("isaac", "libero", "robotwin"),
