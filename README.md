...
| `libero-spatial` · `-object` · `-goal` · `-10` · `-90` | all / mini | — (pose / OSC) | success / steps_taken / ticks, LIBERO's own predicate check | LIBERO on robosuite 1.4.1 |
...
└── benchmarks/                 one file per line — declarations (standard + upstream): engine, body, action prefix, episodes(), metrics, budget
    ...
    └── env/                    what every benchmark is built on
        ...
        ├── metrics.py          NavMetrics (one formula set) · SequenceNavMetrics (GOAT) · VLNVerseMetrics · ManipMetrics (LIBERO, RoboCasa)
...
inside the `libero` package (INSTALL-libero.md). Nor do the RoboCasa lines: a
...
5. RoboCasa (robosuite 1.5 + MuJoCo) — the `robocasa*-*` lines only, in *two* more envs
...
6. `pytest tests` with the data roots set (a GPU and the datasets are needed); the Isaac
   ...
   `EMBODIEDSCORE_RUN_LIBERO_TESTS=1`, the RoboCasa ones with
