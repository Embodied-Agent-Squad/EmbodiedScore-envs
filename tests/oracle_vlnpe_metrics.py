"""The VLNverse evaluator's ``VLNPEMetrics``, arithmetic verbatim.

Source: billzhao1030/VLNVerse @ d444c04,
``vlnverse/projects/internutopia_vln_extension/metrics/vln_pe_metrics.py``.
Only the InternUtopia ``BaseMetric`` base class and the pydantic configs are
replaced by plain objects so it runs without Isaac; the numerics are the
oracle ``test_metrics.py`` checks ``VLNVerseMetrics`` against. Do not "fix"
anything here.
"""
...
class VLNPEMetrics:
    ...
    def __init__(self, config, task_config):  # runtime get data
        self.config = config
