...
``test_robocasa_contracts.py``, the CALVIN lines in ``test_calvin_contracts.py``, the BEHAVIOR line in
...
def test_every_line_has_both_variants():
    ...
    assert len(es.BENCHMARKS) == 2 * len(lines) == 88
    ...
    for line in lines:
        ...
        elif std.engine == "calvin":   # likewise (test_calvin_contracts.py has the rest)
            assert std.body is bodies.CALVIN_STANDARD and std.depth is None
        else:   # behavior: likewise (test_behavior_contracts.py has the rest)
            assert std.engine == "behavior" and std.body is bodies.BEHAVIOR_STANDARD and std.depth is None and std.macro
