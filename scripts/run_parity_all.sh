#!/usr/bin/env bash
# Every parity suite the workspace's legacy runs can feed, one after another.
...
R=parity_reports
D=$(date +%F)
run() { local name=$1; shift; echo "== $name"; $P -u scripts/parity_replay.py "$@" > "$R/${D}_${name}.log" 2>&1; tail -1 "$R/${D}_${name}.log"; }
...
echo "== hmeqa-pose graph log"; $P -u scripts/parity_pose_log.py --run /data/ws_vln/vlnworkspace/outputs/eval_runs/20260615_183412 > $R/${D}_hmeqa-pose_eval_runs_20260615_183412.log 2>&1; tail -1 $R/${D}_hmeqa-pose_eval_runs_20260615_183412.log
